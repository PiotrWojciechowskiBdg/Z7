...
<body>
	<div id="container">
		<div class="title" style="display:inline-block;margin-right:300px;margin-left:300px;">Chmura</div>
		<a href="logout.php"><div class="button" style="display:inline-block;">Wyloguj</div><a/>
		...
		<form action="upload.php<?if(isset($_GET['dir']))echo "?dir=".$_GET['dir']; ?>" method="post" enctype="multipart/form-data">
			...
			<input type="submit" value="Zaladuj" name="submit">
		</form>
		...
		<form action="createDirectory.php<?if(isset($_GET['dir']))echo "?dir=".$_GET['dir']; ?>" method="post" enctype="multipart/form-data">
			...
			<input type="submit" value="Utworz" name="submit">
		</form>
		
		<?php
		$path = './uzytkownicy/'.$_COOKIE['user']."/";
		if(isset($_GET['dir']))
		{
			$path = $path.$_GET['dir'];
			echo "<a href='explorer.php'><div class='button'>Glowny</div></a>";
		}
		...
		foreach (new DirectoryIterator($path) as $file) 
		{
			...
			elseif ($file->isFile()) 
			{
				...
				$_SESSION['path'] = $path;
				echo "
				<td><a href='delete.php?filename=$directory'><div class='buttonsm' id='$directory'><i class=del>Usun</i></div></a></td>
				<td><a href='download.php?filename=$filename'><div class='buttonsm' id='$directory'><i class=dow>Pobierz</i></div></a></td></tr>";
			}
		}
		echo "</table></div>";
	?>
